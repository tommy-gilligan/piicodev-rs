...
mod linux {
    ...
    pub extern "C" fn main() {
        ...
        let mut p15 = P15::new(i2c, i2c_address).unwrap();
        let mut delay = Delay;

        loop {
            if p15.data_ready().unwrap() {
                println!(
                    "{:?} {:?}",
                    p15.read_angle().unwrap().as_degrees(),
                    p15.read_magnitude().unwrap()
                );
            }
            delay.delay_ms(500);
        }
...
mod arm {
    ...
    fn main() -> ! {
        ...
        let mut p15 = P15::new(i2c, 0x1C).unwrap();
